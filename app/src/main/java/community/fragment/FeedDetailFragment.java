...
public class FeedDetailFragment extends CommentEditFragment<List<FeedItem>, FeedDetailPresenter>
    ...
    private View initFeedContentView() {
        ViewFinder headerViewFinder = new ViewFinder(getActivity(), R.layout.feed_detail_header);
        // 构造FeedItemViewHolder用于对Feed进行控制
        mFeedViewHolder = headerViewFinder.findViewById(R.id.feed_content_layout);
        mFeedViewHolder.setShowFavouriteView(true);
        mFeedViewHolder.hideActionButtons();
        mFeedViewHolder.setFeedItem(mFeedItem);

        // Like数量TextView
        mLikeTextView = headerViewFinder.findViewById(R.id.umeng_comm_like_count_tv);
    ...
    private void initRefreshLayout(View headerView) {
        mRefreshLayout = mViewFinder.findViewById(R.id.umeng_comm_feed_refresh_layout);
        ...
        mCommentListView = mViewFinder.findViewById(R.id.umeng_comm_comments_list);
...
//                }
    ...
    private void initActionsView() {
        mActionsLayout = mViewFinder.findViewById(R.id.umeng_comm_action_layout);
        // 操作区域
        mLikeActionLayout = mViewFinder.findViewById(R.id.umeng_comm_like_layout);
        ...
        // 赞的TextView
        mLikeActionView = mViewFinder.findViewById(R.id.umeng_comm_like_action_tv);
        setupLikeView(mFeedItem.isLiked);

        mForwardActionLayout = mViewFinder.findViewById(R.id.umeng_comm_forward_layout);
    ...
    private void setupLikeView(boolean isLiked) {
        if (isLiked) {
            mLikeActionView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.wares_icon_wish_highlighted, 0, 0, 0);
            mLikeTextView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.wares_icon_wish_highlighted, 0, 0, 0);
            mLikeActionView.setText(ResFinder.getString("umeng_comm_unlike"));
        } else {
            mLikeActionView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.wares_icon_wish, 0, 0, 0);
            mLikeTextView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.wares_icon_wish, 0, 0, 0);
