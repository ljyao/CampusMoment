...
import com.chat.views.DropdownListView;
...
public class ChatActivity extends AppCompatActivity implements OnClickListener, DropdownListView.OnRefreshListenerHeader {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.chat_input_sms) {
            if (chat_face_container.getVisibility() == View.VISIBLE) {
                chat_face_container.setVisibility(View.GONE);
            }
            if (chat_add_container.getVisibility() == View.VISIBLE) {
        ...
        } else if (v.getId() == R.id.chat_image_face) {
        ...
        } else if (v.getId() == R.id.chat_send_sms) {
            reply = input.getText().toString();
            if (!TextUtils.isEmpty(reply)) {
                sendChatMsg(reply, null);
                input.setText("");
            }
        } else if (v.getId() == R.id.chat_add) {
            hideSoftInputView();// 隐藏软键盘
        ...
        } else if (v.getId() == R.id.chat_choosepicture) {
            choosepicture();
        } else if (v.getId() == R.id.chat_photo) {
            photo();
        }
    ...
    public void sendPic(String imgPath, final String msg) {
        ...
        cardImageLoader.loadDrawable(CompressType.COMPRESS_DP, 100, imgPath, new ImageCallback() {
            @Override
            public void imageLoaded(Bitmap imageDrawable) {
                ...
                LruCaCheFactory.getEmojiCache().put(ImageName, imageDrawable);
                sendMessageUI(msg);
            }
        });
    }
